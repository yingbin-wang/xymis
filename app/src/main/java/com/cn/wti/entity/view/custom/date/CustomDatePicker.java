...
public class CustomDatePicker {
    ...
    public CustomDatePicker(Context context, ResultHandler resultHandler, String startDate, String endDate) {
        if (isValidDate(startDate, "yyyy-MM-dd HH:mm") && isValidDate(endDate, "yyyy-MM-dd HH:mm")) {
            ...
            try {
                ...
                year = year+100;
                String end = String.valueOf(year)+endDate.substring(4,endDate.length());
            ...
            initDialog();
            initView();
        }else if (isValidDate(startDate, "yyyy-MM-dd") && isValidDate(endDate, "yyyy-MM-dd")) {
            canAccess = true;
            this.context = context;
            this.handler = resultHandler;
            selectedCalender = Calendar.getInstance();
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (type.equals("date")){
                    ...
                    String val = sdf.format(selectedCalender.getTime());
                    handler.handle(val.substring(0,10));
                }else if(type.equals("ny")){
                    sdf = new SimpleDateFormat("yyyy-MM", Locale.CHINA);
                    String val = sdf.format(selectedCalender.getTime());
                    handler.handle(val.substring(0,7));
                }else{
    ...
    public void show(String time,boolean canAccess) {
        if (canAccess) {
            if (isValidDate(time, "yyyy-MM-dd")) {
                if (startCalendar.getTime().getTime() < endCalendar.getTime().getTime()) {
                    canAccess = true;
    ...
    public void showSpecificTime(boolean show,String type) {
        if (canAccess) {
            this.type = type;
            if (show) {
                disScrollUnit();
