 ...
 * 
 */
public class DateValidation extends ValidationExecutor {

	@Override
	public boolean doValidate(Context context,String title, String text) {

		boolean result = DateUtil.isDate(text);
		if (!result) {
			Toast.makeText(context, title+":"+context.getString(R.string.date_text), Toast.LENGTH_SHORT).show();
			return false;
		}
		return true;
	...
	public boolean doValidate(Context context, View view) {
		...
		if (view instanceof EditText_custom){
			...
			boolean result = DateUtil.isDate(v.getText().toString());
			if (!result) {
				Toast.makeText(context, v.getTitle()+":"+context.getString(R.string.date_text), Toast.LENGTH_SHORT).show();
				return false;
			}
		}
